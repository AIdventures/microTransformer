...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v):
        B, T, E = k.size()  # batch size, sequence length, embedding size (n_embd)
        ...
        # Multi-heads: (B, T, E) -> (B, nh, T, hs)
        q = q.view(B, T_Q, self.n_head, E // self.n_head).transpose(1, 2)
        ...
        # re-assemble all head outputs side by side: (B, nh, T, hs) -> (B, T, E)
...
class Transformer(nn.Module):
        ...
        for pn, p in self.encoder.named_parameters():
            if pn.endswith("c_proj.weight"):
                torch.nn.init.normal_(
                    p, mean=0.0, std=0.02 / math.sqrt(2 * config.n_encoder_layer)
                )

        for pn, p in self.decoder.named_parameters():
            if pn.endswith("c_proj.weight"):
                torch.nn.init.normal_(
                    p, mean=0.0, std=0.02 / math.sqrt(2 * config.n_decoder_layer)
                )
    ...
    def encoder_forward(self, src):
        ...
        pos_emb = self.encoder.wpe(
            torch.arange(0, src.size(1), dtype=torch.long, device=device)
    ...
    def decoder_forward(self, enc_out, tgt):
        ...
        pos_emb = self.decoder.wpe(
            torch.arange(0, tgt.size(1), dtype=torch.long, device=device)
        )  # positional embeddings (t, n_embd)
    ...
    def generate(
        ...
        Args:
            src (list[str]): the source sequence
        ...
        with torch.no_grad():
            ...
            src = (
                torch.tensor(tokenize_sentence(src, VOCABULARY)).unsqueeze(0).to(device)
            )
